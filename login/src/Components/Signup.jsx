...
function Signup() {
    ...
    return (
        ...
        <NavBars/>      
        <div className={`flex-1 flex   w-screen  items-center justify-center font-questrial overflow-hidden p-4 transition-colors duration-500 ${theme === 'dark' ? 'bg-slate-800' : 'bg-primary text-black'} transition-colors duration-500`}>
           
            <Toaster position="top-center" /> 
            
            <div className="signup-card w-full max-w-6xl rounded-2xl h-full max-h-[85vh] grid grid-cols-1 md:grid-cols-2 shadow-xl overflow-hidden  relative">
                ...
                <div className={`signup-info ${theme === 'dark' ? "bg-[url('/images/dark-blue-signup.jpg')]" : "bg-[url('/images/light-green-signup.jpg')]"} bg-cover bg-center text-white p-8 md:p-12 relative flex flex-col justify-between h-full transition-all duration-500`}>
                    ...
                    <div className="flex justify-start gap-3 mt-4">
                        ...
                        <div className={`w-28 h-28 rounded-2xl p-4 flex flex-col justify-between shadow-lg transition-colors ${theme === 'dark' ? 'bg-white text-black' : 'bg-white text-black'}`}>
                            ...
                            <p className="text-sm font-bold leading-tight">{t('signup_account')}</p>
                        </div>
                        {/* Step 2 */}
                        <div className={`w-28 h-28 rounded-2xl p-4 flex flex-col justify-between opacity-80 transition-colors ${theme === 'dark' ? 'bg-slate-800 text-white border border-slate-700' : 'bg-emerald-700 text-white'}`}>
                            ...
                            <span className="text-sm font-medium leading-tight">{t('step_2')}</span>
                        </div>
                        {/* Step 3 */}
                        <div className={`w-28 h-28 rounded-2xl p-4 flex flex-col justify-between opacity-80 transition-colors ${theme === 'dark' ? 'bg-slate-800 text-white border border-slate-700' : 'bg-emerald-700 text-white'}`}>
                            <span className="text-white/70">3</span>
                            <span className="text-sm font-medium leading-tight">{t('step_3')}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className={`signup-form p-8 md:p-12 flex flex-col justify-center h-full overflow-y-auto transition-colors duration-500 ${theme === 'dark' ? 'bg-slate-900 text-white' : 'bg-white text-black'}`}>
                    <h2 className={`text-3xl font-extrabold mb-6 ${theme === 'dark' ? 'text-white' : 'text-black'}`}>{t('signup_account')}</h2>
                    
                    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className={`block mb-1 text-sm font-medium ${theme === 'dark' ? 'text-[#a4b5aa]' : 'text-gray-600'}`}>{t('first_name')}</label>
                                <input 
                                    className={`w-full p-3 padding-inline-end-12 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 transition-all ${theme === 'dark' ? 'bg-slate-800 text-white border border-slate-700' : 'bg-gray-100 text-black'}`} 
                                    type="text" name="firstName" placeholder="eg John" value={formData.firstName} onChange={handleInput} required 
                                />
                            </div>
                            <div>
                                <label className={`block mb-1 text-sm font-medium ${theme === 'dark' ? 'text-[#a4b5aa]' : 'text-gray-600'}`}>{t('last_name')}</label>
                                <input 
                        ...
                        <div>
                            <label className={`block mb-1 text-sm font-medium ${theme === 'dark' ? 'text-[#a4b5aa]' : 'text-gray-600'}`}>{t('email')}</label>
                            <input 
                                className={`w-full p-3 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 transition-all ${theme === 'dark' ? 'bg-slate-800 text-white border border-slate-700' : 'bg-gray-100 text-black'}`} 
                        ...
                        <div>
                            <label className={`block mb-1 text-sm font-medium ${theme === 'dark' ? 'text-[#a4b5aa]' : 'text-gray-600'}`}>{t('create_password')}</label>
                        ...
                        <div>
                            <label className={`block mb-1 text-sm font-medium ${theme === 'dark' ? 'text-[#a4b5aa]' : 'text-gray-600'}`}>{t('confirm_password')}</label>
                            <div className="relative">
                                <input 
                                    className={`w-full p-3 padding-inline-end-12 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 transition-all ${theme === 'dark' ? 'bg-slate-800 text-white border border-slate-700' : 'bg-gray-100 text-black'}`} 
                                    type={showPassword ? "text" : "password"} 
                    ...
                    <p className={`text-center text-sm mt-6 ${theme === 'dark' ? 'text-[#a4b5aa]' : 'text-gray-600'}`}>
                        {t('already_have_account')} <button onClick={() => navigate('/login')} className={`font-semibold hover:underline ${theme === 'dark' ? 'text-white' : 'text-emerald-700'}`}>{t('log_in')}</button>
                    </p>
                </div>
            </div>
